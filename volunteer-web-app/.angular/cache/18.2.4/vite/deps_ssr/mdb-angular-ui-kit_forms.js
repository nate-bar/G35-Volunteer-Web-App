...
var MdbFormControlComponent = class _MdbFormControlComponent {
  ...
  ngAfterContentInit() {
    ...
    if (this._label) {
      this._contentObserver.observe(this._label.nativeElement).pipe((0, import_operators.takeUntil)(this._destroy$)).subscribe(() => {
        this._updateBorderGap();
      });
    }
    this._formControl.stateChanges.pipe((0, import_operators.takeUntil)(this._destroy$)).subscribe(() => {
      this._updateLabelActiveState();
      if (this._label) {
        this._updateBorderGap();
      }
    ...
    this._ngZone.runOutsideAngular(() => {
      this._ngZone.onStable.pipe((0, import_operators.takeUntil)(this._destroy$)).subscribe(() => {
